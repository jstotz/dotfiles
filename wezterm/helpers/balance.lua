-- Based on: https://gist.github.com/fcpg/eb3c05be5b480f4cad767199dac5cecd
...
local function walk_siblings(axis, tab, window, pane, do_func)
  ...
  for _, step_dir in ipairs({ "prev", "next" }) do
    ...
    window:perform_action(
      wezterm.action.ActivatePaneDirection(
        step_dir == "prev" and prev_dir or next_dir
      ),
      tab:active_pane()
    )
    -- [[DEBUG
    ...
    -- [[DEBUG
    wezterm.log_info(
      string.format("back to initial pane: %s", tab:active_pane())
    )
    --]]
  end
